...
<%i++;}}%>
  ...
  <div id="center">
  	<p id="title">【易索新闻】</p>
	<p class="right more"><a href="/aesop/front/article/ArticleIndex.jsp">更多...</a></p>
	<%
    List articleList=(List)request.getAttribute("articleList");
    if(articleList==null||articleList.size()==0){
	%>
	<p class="prompt">目前还未发表任何文章！</p>
	<%} else {
    int i=0;
	...
	%>
 	...
 	<p>▲<a href="/aesop/blog/ArticleAction?action=read&id=<%=articleSingle.getId()%>">
 		<b><%=articleSingle.getTitle()%></b></a>[<%=articleSingle.getCreate()%>]
 	</p>
 	<p class="right read"><a href="/aesop/blog/ArticleAction?action=read&id=<%=articleSingle.getId()%>">阅读全文</a>
 	</p>
 	<%=articleSingle.getContent(50) %><br />
